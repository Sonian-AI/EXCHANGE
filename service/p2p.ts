...
export const getDynamicP2PBankFormListApi = async () => {
  const { data } = await p2pResuest.get("/admin-payment-method");
  return data;
};

export const addEditDynamicP2PBankDetailsApi = async (payload: any) => {
  const { data } = await p2pResuest.post("/payment-method", payload);
  return data;
};

export const getUserP2PBankListApi = async () => {
  const { data } = await p2pResuest.get("/payment-method");
  return data;
};

export const deleteP2PBankItemApi = async (id: any) => {
  const { data } = await p2pResuest.get(`/payment-method-delete-${id}`);
  return data;
};

export const getP2PBankDetailsApi = async (id: any) => {
  const { data } = await p2pResuest.get(`/details-payment-method-${id}`);
  return data;
};
