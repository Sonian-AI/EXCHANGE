...
export const UpdateUserProfileAction = (user: any) => async (dispatch: any) => {
  const response = await UpdateUserProfileByToken(user);
  if (response.success === true) {
    toast.success(response.message, {
      position: "top-right",
      autoClose: 5000,
...
export const SocialSigninAction =
  (credentials: any, setProcessing: any) => async (dispatch: any) => {
    setProcessing(true);
    const response = await socialSigninApi(credentials);
    const responseMessage = response.message;
