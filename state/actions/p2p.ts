...
import {
  ...
  getUserP2PBankListApi,
  getDynamicP2PBankFormListApi,
  addEditDynamicP2PBankDetailsApi,
  deleteP2PBankItemApi,
  getP2PBankDetailsApi,
} from "service/p2p";
import {
  setP2pDetails,
  setP2pDetailsOrder,
...
export const useGetDynamicP2PBankFormList = () => {
  ...
  const fetchBankFormLists = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await getDynamicP2PBankFormListApi();
...
export const useAddEditP2PBankDetailsAction = (bank_id: any) => {
  ...
  const {
    bankFormLists,
    error,
    loading: isDynamicFormLoading,
  } = useGetDynamicP2PBankFormList();

  const { bankDetails, loading: isBankDetailsLoading } =
    useGetP2PBankDetails(bank_id);

  const formik = useFormik({
    initialValues: {
      id: bank_id || "",
      access_type: BANK_FORM_ACCESS_TYPE.P2P,
      form_id: bankDetails?.form_id || "",
      ...bankFormLists.reduce((acc: any, bank: any) => {
        bank.bank_form?.fields?.forEach((f: any) => {
          acc[f.slug] = bankDetails?.bank?.[f.slug]?.value || "";
        });
        return acc;
      }, {}),
    },
    validate: (values) => {
      ...
      if (!values.form_id) {
        errors.form_id = t("Please select a bank");
      }

      if (values.form_id) {
        const selectedBank: any = bankFormLists.find(
          (b: any) => b.bank_form.id == values.form_id
        );

        if (selectedBank?.bank_form?.fields?.length) {
          selectedBank.bank_form.fields.forEach((field: any) => {
            if (field?.required == 1 && !values[field.slug]) {
              errors[field.slug] = t(`${field.title} is required`);
            }

            if (field.data_type === "email" && values[field.slug]) {
              const emailRegex = /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i;
              if (!emailRegex.test(values[field.slug])) {
                errors[field.slug] = t("Invalid email address");
              }
            }
          });
        }
  ...
  const selectedBank: any = bankFormLists.find(
    (b: any) => b.bank_form.id == formik.values.form_id
  );

  const handleSubmit = useCallback(async (payload: any) => {
    try {
      setLoading(true);
      const response = await addEditDynamicP2PBankDetailsApi(payload);
...
export const useGetP2PBankListAction = () => {
  const [loading, setLoading] = useState(false);
  ...
  const fetchUserBankLists = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await getUserP2PBankListApi();
      setUserBankLists(response?.data?.data);
    } catch (err) {
      setError(err instanceof Error ? err : new Error("An error occurred"));
      setUserBankLists([]);
    } finally {
...
export const useDeleteP2PBank = () => {
  const [loading, setLoading] = useState(false);

  const deleteBankAction = useCallback(async (id: any) => {
    try {
      setLoading(true);
      const response = await deleteP2PBankItemApi(id);
      if (response.success) {
        toast.success(response.message);
      } else {
        toast.error(response.message);
...
export const useGetP2PBankDetails = (id?: string | number) => {
  const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        setLoading(true);
        const res = await getP2PBankDetailsApi(id);
        if (res.success) {
