...
const DeposiAndWithdraw = ({ withdrawFaq, depositFaq }: any) => {
  ...
  const handleWithdrawAndDeposit = async (
    actionType: string,
    coin_type: any
  ) => {
    if (actionType === MY_WALLET_DEPOSIT_TYPE) {
      const response = await WalletDepositApiAction(router.query.coin_type);
      if (response.success === true) {
        setResponseData({
          ...response?.data,
          deposit: response?.data.wallet,
      ...
      const response = await WalletWithdrawApiAction(router.query.coin_type);

      if (response.success === true) {
        setResponseData({
          ...response?.data,
          withdraw: response?.data.wallet,
  ...
  useEffect(() => {
    ...
    if (WithdrawAndDeposit === false) {
      handleWithdrawAndDeposit(String(router.query.id), router.query.coin_type);
  ...
  return (
    <>
      <WalletLayout>
        ...
        {router.query.id === MY_WALLET_DEPOSIT_TYPE &&
          networkTypeCheckHandler(responseData?.network?.id) && (
            <CheckDeposit />
          )}
